...
export function useRegistroOtFallidasScreen() {
  ...
  const openExcluirSeleccionadas = () => { const seleccionadas = rows.value.filter((row) => row.selected && row.excluida !== 'S'); if (!seleccionadas.length) return showAlert('Alerta', 'Debe seleccionar al menos una OT para excluir.', 'warning'); excludePopup.rows = seleccionadas; excludePopup.motivo = ''; excludePopup.nota = ''; excludePopup.x = Math.max(window.innerWidth / 2 - 285, 20); excludePopup.y = 120; excludePopup.show = true }
  const confirmarExcluir = async () => { if (!excludePopup.motivo) return showAlert('Alerta', 'Debe seleccionar un motivo.', 'warning'); loading.value = true; try { await excludeRegistroOtFallidas({ ots: excludePopup.rows.map((row) => row.nroOt), motivo: excludePopup.motivo, nota: excludePopup.nota }); excludePopup.rows.forEach((row) => { row.excluida = 'S'; row.selected = false; row.recovered = false; row.motivoExclusion = excludePopup.motivo; row.incluir = 'RECUPERAR'; row.nota = excludePopup.nota || row.nota }); excludePopup.show = false } finally { loading.value = false } }
