...
export function useRegistroOtFallidasScreen() {
  ...
  const buscar = async () => { loading.value = true; openGrid.value = true; try { rows.value = await searchRegistroOtFallidas({ ...filters }); page.value = 1; if (!rows.value.length) showAlert('Alerta', 'No hay datos para la consulta efectuada') } finally { loading.value = false } }
  ...
  const confirmarExcluir = async () => { if (!excludePopup.motivo) return showAlert('Alerta', 'Debe seleccionar un motivo.'); loading.value = true; try { await excludeRegistroOtFallidas({ ots: excludePopup.rows.map((row) => row.nroOt), motivo: excludePopup.motivo, nota: excludePopup.nota }); excludePopup.rows.forEach((row) => { row.excluida = 'S'; row.selected = false; row.recovered = false; row.motivoExclusion = excludePopup.motivo; row.incluir = 'RECUPERAR'; row.nota = excludePopup.nota || row.nota }); excludePopup.show = false } finally { loading.value = false } }
