  ...
  <div class="materiales-ot-page">
      ...
      <div v-show="filtersOpen" class="fm-panel-content filters-content">
        <div class="filters-row">
          ...
          <div class="fm-field">
            ...
            <select v-model="filters.centro" class="form-control" :class="{ 'input-error': validation.centro }">
              <option value="">None selected</option>
              <option value="PNT1">PNT1</option>
              <option value="PNL1">PNL1</option>
              <option value="OC15">OC15</option>
            </select>
            <span v-if="validation.centro" class="error-text">Al menos un campo es obligatorio</span>
          </div>
        ...
        <div class="filters-row second-row">
          <div class="fm-field tecnico-field">
            <label>Técnico</label>
            <div class="tecnico-wrapper">
              <input v-model.trim="filters.tecnico" class="form-control" :class="{ 'input-error': validation.tecnico }" :disabled="todosTecnicos" />
              <button type="button" class="todos-btn" :class="{ active: todosTecnicos }" @click="toggleTodos">TODOS</button>
            </div>
            <span v-if="validation.tecnico" class="error-text">Al menos un campo es obligatorio</span>
          </div>

          <div class="fm-field">
            <label>Nro. Ot</label>
            <input v-model.trim="filters.ot" class="form-control" :class="{ 'input-error': validation.ot }" :disabled="todosTecnicos" />
            <span v-if="validation.ot" class="error-text">Al menos un campo es obligatorio</span>
          </div>
    ...
    <section class="fm-panel results-panel">
      <div class="fm-panel-header" @click="resultsOpen = !resultsOpen">
        <span>OTS PENDIENTES DE GESTIÓN DE MATERIALES</span>
      ...
      <div v-show="resultsOpen" class="fm-panel-content grid-content">
          ...
          <table class="ot-grid">
            ...
            <tbody>
              <tr v-for="row in paginatedRows" :key="row.id">
                ...
                <td :title="row.nroOt">{{ row.nroOt }}</td>
                <td :title="row.estadoOt">{{ row.estadoOt }}</td>
                <td :title="row.fechaUltMod">{{ row.fechaUltMod }}</td>
                <td :title="row.horaUltMod">{{ row.horaUltMod }}</td>
                <td :title="row.tecnico">{{ row.tecnico }}</td>
                <td :title="row.superior">{{ row.superior }}</td>
                <td :title="row.centro">{{ row.centro }}</td>
                <td :title="row.almacen">{{ row.almacen }}</td>
                <td :title="row.direccion">{{ row.direccion }}</td>
                <td :title="row.codTarea">{{ row.codTarea }}</td>
              </tr>
        ...
        <div class="ot-grid-footer">
          <div class="footer-icons">
            <button type="button" class="plain-icon" title="Descargar" :disabled="filteredRows.length === 0" @click="exportRows">
              <span class="material-icons">file_download</span>
            </button>
            <button type="button" class="plain-icon" title="Gestionar" :disabled="filteredRows.length === 0" @click="showNoResultsAlert">
              <span class="material-icons">build</span>
            </button>
            <button type="button" class="plain-icon" title="Configurar" @click="showNoResultsAlert">
      ...
      <div class="pretty-alert">
        ...
        <div class="pretty-alert-body">
          ...
          <div>
            ...
            <p>La búsqueda no devolvió OTs pendientes de gestión de materiales para los filtros ingresados.</p>
          </div>
        </div>
...
const columns = [
  { field: 'nroOt', label: 'NRO. OT', width: '180px', minWidth: '140px' },
  { field: 'estadoOt', label: 'ESTADO OT', width: '180px', minWidth: '140px' },
  { field: 'fechaUltMod', label: 'FECHA ULT. MODIFICACIÓN', width: '180px', minWidth: '150px' },
  { field: 'horaUltMod', label: 'HORA ULT. MODIFICACIÓN', width: '180px', minWidth: '150px' },
  { field: 'tecnico', label: 'TÉCNICO', width: '180px', minWidth: '140px' },
  { field: 'superior', label: 'SUPERIOR', width: '180px', minWidth: '140px' },
  { field: 'centro', label: 'CENTRO', width: '180px', minWidth: '130px' },
  { field: 'almacen', label: 'ALMACÉN', width: '180px', minWidth: '130px' },
  { field: 'direccion', label: 'DIRECCIÓN', width: '180px', minWidth: '150px' },
  { field: 'codTarea', label: 'COD TAREA', width: '180px', minWidth: '130px' }
]
...
const clearValidation = () => {
  validation.centro = false
  validation.tecnico = false
  validation.ot = false
}

const toggleTodos = () => {
  todosTecnicos.value = !todosTecnicos.value
  if (todosTecnicos.value) {
    filters.tecnico = ''
    filters.ot = ''
  }
  clearValidation()
...
const handleSearch = () => {
  ...
  if (!hasSearchCriteria()) {
    validation.centro = true
    validation.tecnico = true
    validation.ot = true
    return
  }
...
const filteredRows = computed(() => rows.value.filter((row) => columns.every((column) => {
...
const exportRows = () => {
  ...
  const csvRows = filteredRows.value.map((row) => columns.map((column) => row[column.field]))
  downloadCsv('ots_pendientes_gestion_materiales.csv', buildCsv(headers, csvRows))
}
