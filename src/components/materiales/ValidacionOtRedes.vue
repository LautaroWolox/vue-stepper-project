      ...
      <div v-show="openFilters" class="fm-panel-content filters-content">
        ...
        <div class="filters-row bottom-row">
          <div class="fm-field autocomplete-field">
            <label>Nro. Ot</label>
            <input class="form-control" :class="{ disabled: isNroOtDisabled }" v-model.trim="filters.nroOt" :disabled="isNroOtDisabled" autocomplete="off" @focus="openNroOtAutocomplete" @input="handleNroOtInput" @blur="closeNroOtAutocomplete" @keydown.down.prevent="moveNroOtSuggestion(1)" @keydown.up.prevent="moveNroOtSuggestion(-1)" @keydown.enter.prevent="confirmNroOtSuggestion" @keydown.esc.prevent="showNroOtSuggestions = false" />
            <div v-if="showNroOtSuggestions && filteredNroOtSuggestions.length" class="ot-autocomplete-panel">
              <button v-for="(item, index) in filteredNroOtSuggestions" :key="item" type="button" class="ot-suggestion" :class="{ active: index === activeSuggestionIndex }" @mousedown.prevent="selectNroOt(item)">
                <span>{{ item }}</span>
              </button>
            </div>
          </div>
...
const showNroOtSuggestions = ref(false)
const activeSuggestionIndex = ref(0)
const filters = reactive({ regiones: [], subRegiones: [], centros: [], almacenes: [], nroOt: '', fechaDesde: '', fechaHasta: '' })
const otAutocompleteOptions = ['014000001','014000002','014000003','014-RED-9000','014-RED-9001','014-RED-9002','OT-014-0001','OT-014-0002','OT-RED-09000','OT-RED-09001','OT-RED-09002','OT-RED-09003','OT-RED-09004','OT-RED-09005','OT-RED-09006','OT-RED-09007','M00966068','M00981724','M00981725','M00981726','J000000000000325296','JOP68807891','JOP68807890','JOP101450834','ICD12482236','ICD12273196','707405355','706020499']
...
const filteredNroOtSuggestions = computed(() => {
  const term = filters.nroOt.trim().toLowerCase()
  if (!term || isNroOtDisabled.value) return []
  return otAutocompleteOptions.filter(item => item.toLowerCase().includes(term)).slice(0, 10)
})
const openNroOtAutocomplete = () => { showNroOtSuggestions.value = filteredNroOtSuggestions.value.length > 0; activeSuggestionIndex.value = 0 }
const handleNroOtInput = () => { showNroOtSuggestions.value = filteredNroOtSuggestions.value.length > 0; activeSuggestionIndex.value = 0 }
const closeNroOtAutocomplete = () => { setTimeout(() => { showNroOtSuggestions.value = false }, 140) }
const selectNroOt = value => { filters.nroOt = value; showNroOtSuggestions.value = false; filters.fechaHasta = '' }
const moveNroOtSuggestion = direction => { if (!filteredNroOtSuggestions.value.length) return; showNroOtSuggestions.value = true; const max = filteredNroOtSuggestions.value.length - 1; activeSuggestionIndex.value = Math.min(max, Math.max(0, activeSuggestionIndex.value + direction)) }
const confirmNroOtSuggestion = () => { if (!showNroOtSuggestions.value || !filteredNroOtSuggestions.value.length) return; selectNroOt(filteredNroOtSuggestions.value[activeSuggestionIndex.value]) }
...
const limpiar = () => { filters.regiones = []; filters.subRegiones = []; filters.centros = []; filters.almacenes = []; filters.nroOt = ''; filters.fechaDesde = ''; filters.fechaHasta = ''; centroError.value = ''; generalError.value = ''; showNroOtSuggestions.value = false; rows.value = []; clearGridFilters(); openFilters.value = true }
