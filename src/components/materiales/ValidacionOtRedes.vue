    ...
    <section class="fm-panel redes-filter-panel">
      ...
      <div v-show="openFilters" class="fm-panel-content filters-content">
        <div class="filters-row top-row">
          <MultiSelectBox label="Región" :items="regionOptions" v-model="filters.regiones" :allow-select-all="true" placeholder="None selected" />
          <MultiSelectBox label="Sub Región" :items="subRegionOptions" v-model="filters.subRegiones" :allow-select-all="true" placeholder="None selected" />
          <MultiSelectBox label="Centro Logístico" :items="centroOptions" v-model="filters.centros" :allow-select-all="true" placeholder="None selected" :error="centroError" />
...
const MultiSelectBox = defineComponent({
  props: { label: String, items: { type: Array, default: () => [] }, modelValue: { type: Array, default: () => [] }, allowSelectAll: { type: Boolean, default: true }, placeholder: { type: String, default: 'None selected' }, error: { type: String, default: '' } },
  ...
  setup(props, { emit }) {
    ...
    const toggle = item => { const selected = [...props.modelValue]; const index = selected.indexOf(item); if (index >= 0) selected.splice(index, 1); else selected.push(item); emit('update:modelValue', selected) }
