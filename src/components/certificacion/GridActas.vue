<template>
  <div class="actas-grid-wrapper">
        ...
        <tbody>
          <tr v-for="row in paginatedData" :key="row.id" @click="selectedRow = selectedRow?.id === row.id ? null : row" :class="{ selected: selectedRow?.id === row.id }">
            <td v-for="column in columns" :key="`${row.id}-${column.field}`" :title="row[column.field]" :style="columnStyle(column.field)">
              <span v-if="column.field === 'nro_acta'" class="acta-link" @click.stop="$emit('open-acta', row)">{{ row[column.field] }}</span>
          ...
          <tr v-if="paginatedData.length === 0">
            <td :colspan="columns.length" class="empty-cell">No hay resultados con estos filtros.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <ExportExcelDialog :show="showExportDialog" description="Elegí si querés exportar toda la grilla de actas o solo lo visible, y todos los campos o solo los campos llenos." @close="showExportDialog = false" @confirm="confirmExportExcel" />
  </div>
</template>
...
const columns = [
  { field: 'nro_acta', label: 'NRO_ACTA', width: 130, minWidth: 70 },
  { field: 'nd', label: 'ND ASOCIADA', width: 130, minWidth: 70 },
  { field: 'nc', label: 'NC ASOCIADA', width: 130, minWidth: 70 },
  { field: 'estado', label: 'ESTADO_ACTA', width: 130, minWidth: 75 },
  { field: 'periodo', label: 'PERIODO', width: 145, minWidth: 80 },
  { field: 'anio', label: 'AÑO', width: 90, minWidth: 55 },
  { field: 'fecha_crea', label: 'FECHA_CREACION', width: 150, minWidth: 80 },
  { field: 'fecha_cierre', label: 'FECHA_CIERRE', width: 150, minWidth: 80 },
  { field: 'contratista', label: 'CONTRATISTA', width: 170, minWidth: 90 },
  { field: 'pais', label: 'PAIS', width: 110, minWidth: 60 },
  { field: 'provincia', label: 'PROVINCIA', width: 135, minWidth: 75 },
  { field: 'region', label: 'REGION', width: 125, minWidth: 70 },
  { field: 'tipo_contrato', label: 'TIPO_CONTRATO', width: 135, minWidth: 80 },
  { field: 'sociedad', label: 'SOCIEDAD', width: 155, minWidth: 80 },
  { field: 'usuario', label: 'USUARIO_CIERRE', width: 135, minWidth: 80 },
  { field: 'valoracion', label: 'VALORACION', width: 125, minWidth: 70 }
]
...
const confirmExportExcel = ({ scope, columnMode }) => {
  ...
  downloadExcel({ filename: 'Consulta_Actas.xls', title: 'Consulta Actas', columns, rows, mode: columnMode })
  showExportDialog.value = false
}
const toggleSort = (field) => {
  if (sortState.field !== field) { sortState.field = field; sortState.direction = 'asc' }
  else if (sortState.direction === 'asc') sortState.direction = 'desc'
  else { sortState.field = ''; sortState.direction = '' }
  currentPage.value = 1
}
...
const startColumnResize = (event, field) => {
  ...
  document.body.classList.add('actas-column-resizing')
...
const stopColumnResize = () => {
  resizingField = null
  document.body.classList.remove('actas-column-resizing')
...
<style scoped>
.actas-grid-wrapper { width: 100%; }
.actas-table-wrap { overflow: auto; min-height: 500px; max-height: 620px; border: 1px solid #d1d1d1; border-bottom: 0; }
.actas-grid { width: max-content; min-width: 100%; border-collapse: collapse; table-layout: fixed; font-size: 12px; }
.actas-grid th, .actas-grid td { border-right: 1px solid #c9d3da; border-bottom: 1px solid #dce3e8; padding: 7px 8px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.actas-grid thead th { position: sticky; background: #f4f7f9; color: #2c4050; font-weight: 600; font-size: 11px; z-index: 5; }
.header-row th { top: 0; height: 36px; }
.filter-row th { top: 36px; height: 34px; background: #fff; padding: 4px 5px; }
.sort-button { width: 100%; height: 100%; border: 0; background: transparent; color: inherit; display: flex; align-items: center; justify-content: space-between; gap: 6px; padding: 0; text-align: left; font: inherit; cursor: pointer; }
.sort-button:hover { color: #008fa1; }
.sort-icons { display: inline-flex; flex-direction: column; color: #aab8c0; font-size: 8px; line-height: 7px; min-width: 9px; }
.sort-icons .active { color: #00a9bd; }
.resize-handle { position: absolute; top: 0; right: -4px; width: 10px; height: 100%; cursor: col-resize; z-index: 10; border-right: 2px solid transparent; }
.resize-handle:hover { background: rgba(0,188,212,.16); border-right-color: #00bcd4; }
.col-filter { display: grid; grid-template-columns: 14px minmax(36px, 1fr) 13px; align-items: center; gap: 5px; }
.col-filter-input { width: 100%; height: 24px; border: 1px solid #c7d1d8; border-radius: 3px; padding: 3px 6px; font-size: 11px; }
.col-filter-input:focus { outline: none; border-color: #00bcd4; box-shadow: 0 0 0 2px rgba(0,188,212,.14); }
.col-filter button { appearance: none; border: 0; background: transparent; cursor: pointer; font-weight: 700; padding: 0; color: #000; }
.actas-grid tbody tr { cursor: pointer; }
.actas-grid tbody tr:hover td { background: #edfafd; }
.actas-grid tbody tr.selected td { background: #9eeff7; color: #003f47; }
.empty-cell { text-align: center !important; padding: 34px !important; color: #607d8b; }
.acta-link { color: #00a9bd; font-weight: 700; cursor: pointer; }
.acta-link:hover { color: #008fa1; text-decoration: underline; }
.estado-badge { display: inline-flex; min-width: 82px; justify-content: center; border-radius: 12px; padding: 3px 9px; font-size: 11px; font-weight: 700; }
...
.actas-grid-footer { min-height: 38px; display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; gap: 12px; padding: 6px 10px; border: 1px solid #d1d1d1; background: #fff; }
...
.plain-icon .material-icons, .page-icon .material-icons { font-size: 17px; }
...
.footer-pagination { display: flex; align-items: center; justify-content: center; gap: 6px; font-size: 12px; white-space: nowrap; }
.page-input { width: 40px; height: 24px; border: 1px solid #d8d8d8; border-radius: 3px; text-align: center; font-weight: 400; }
.page-size { height: 26px; border: 1px solid #d8d8d8; border-radius: 3px; font-size: 12px; }
.footer-count { justify-self: end; font-size: 12px; white-space: nowrap; }
@media (max-width: 900px) { .actas-grid-footer { grid-template-columns: 1fr; justify-items: center; } .footer-count { justify-self: center; } }
@media (max-width: 560px) { .footer-pagination { flex-wrap: wrap; } }
</style>

<style>
body.actas-column-resizing, body.actas-column-resizing * { cursor: col-resize !important; user-select: none !important; }
</style>
