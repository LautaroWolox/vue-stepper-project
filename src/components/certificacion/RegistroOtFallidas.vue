      ...
      <div v-show="openGrid" class="thin-content grid-box">
        <div class="grid-scroll">
          <table class="fallidas-grid">
            ...
            <tbody>
              <tr v-for="row in pageRows" :key="row.id" :class="{ excluded: row.excluida === 'S', recovered: row.recovered, selected: row.selected }">
                ...
                <td v-for="col in columns" :key="`${row.id}-${col.field}`">
                  ...
                  <button v-else-if="col.field === 'incluir'" class="icon-cell include-icon" :disabled="row.excluida !== 'S'" title="Incluir / recuperar OT" @click="openIncluir(row)"><span class="material-icons">settings_backup_restore</span></button>
