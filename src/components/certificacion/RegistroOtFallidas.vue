<template>
  <div class="fallidas-page">
    <section class="thin-accordion">
      ...
      <div v-show="openFilters" class="thin-content filter-box">
        <div class="filters-grid">
          ...
          <label>Contratista<FmTurquoiseSelect v-model="filters.contratista" :options="contratistaOptions" class="filter-select" /></label>
          <label>Descripcion Error<input v-model.trim="filters.descripcionError" /></label>
          <label>Excluida<FmTurquoiseSelect v-model="filters.excluida" :options="excluidaOptions" class="filter-select" /></label>
          <label>País<FmTurquoiseSelect v-model="filters.pais" :options="paisOptions" class="filter-select" /></label>
        </div>
    ...
    <div v-if="includePopup.show" class="float-modal include-modal" :style="modalStyle(includePopup)">
      ...
      <div class="float-body"><p>¿Confirma que desea recuperar la OT seleccionada?</p><label>Motivo<FmTurquoiseSelect v-model="includePopup.motivo" :options="motivoOptions" class="popup-select" /></label><label>Nota<textarea v-model="includePopup.nota" placeholder="Opcional" rows="3"></textarea></label></div>
    ...
    <div v-if="excludePopup.show" class="float-modal include-modal" :style="modalStyle(excludePopup)">
      ...
      <div class="float-body"><p>¿Confirma que desea excluir la OT seleccionada?</p><label>Motivo<FmTurquoiseSelect v-model="excludePopup.motivo" :options="motivoOptions" class="popup-select" /></label><label>Nota<textarea v-model="excludePopup.nota" placeholder="Opcional" rows="3"></textarea></label></div>
...
const contratistaOptions = [{ value: '', label: '' }, ...contratistas.map((item) => ({ value: item, label: item }))]
...
const motivoOptions = [{ value: '', label: '' }, ...motivos.map((item) => ({ value: item, label: item }))]
const perPageOptions = [{ value: 10, label: '10' }, { value: 50, label: '50' }, { value: 100, label: '100' }, { value: 500, label: '500' }]
...
const buscar = async () => { loading.value = true; try { rows.value = await searchRegistroOtFallidas({ ...filters }); page.value = 1; if (!rows.value.length) showAlert('Alerta', 'No hay datos para la consulta efectuadaa') } finally { loading.value = false } }
