  ...
  <div class="nota-credito-page">
    <div v-if="currentView === 'search'">
      <section class="fm-panel">
        ...
        <div class="fm-panel-content filters-content" v-show="openFilters"><div class="filters-row first-row"><div class="fm-field"><label>Provincia</label><select class="form-control" v-model="form.provincia"><option value=""></option><option v-for="provincia in provincias" :key="provincia" :value="provincia">{{ provincia }}</option></select></div><div class="fm-field"><label>Contratista Acta</label><select class="form-control" v-model="form.contratista"><option value=""></option><option v-for="item in contratistas" :key="item" :value="item">{{ item }}</option></select></div><div class="fm-field"><label>Sociedad Acta</label><select class="form-control" v-model="form.sociedad"><option value=""></option><option v-for="item in sociedades" :key="item" :value="item">{{ item }}</option></select></div><div class="fm-field"><label>Tipo de Contrato</label><select class="form-control" v-model="form.tipo_contrato"><option value=""></option><option v-for="item in tiposContrato" :key="item" :value="item">{{ item }}</option></select></div></div><div class="filters-row second-row"><div class="fm-field small-field"><label>Año</label><select class="form-control" v-model="form.anio"><option value=""></option><option value="2026">2026</option><option value="2025">2025</option><option value="2024">2024</option><option value="2023">2023</option></select></div><div class="fm-field"><label>Periodo</label><select class="form-control" v-model="form.periodo"><option value=""></option><option value="Del 16/05 al 15/06">Del 16/05 al 15/06</option><option value="Del 16/03 al 15/04">Del 16/03 al 15/04</option><option value="Del 16/01 al 15/02">Del 16/01 al 15/02</option><option value="Del 16/11 al 15/12">Del 16/11 al 15/12</option></select></div><div class="fm-field"><label>Estado</label><select class="form-control" v-model="form.estado"><option value=""></option><option value="En Curso">En Curso</option><option value="Cerrado">Cerrado</option></select></div></div><div class="filters-row third-row"><div class="fm-field"><label>Nota de Credito</label><input type="text" class="form-control" v-model.trim="form.nota_credito" /></div><div class="fm-field"><label>N° de OT</label><input type="text" class="form-control" v-model.trim="form.nro_ot" /></div><div class="fm-field"><label>Nro Acta Asociada</label><input type="text" class="form-control" v-model.trim="form.acta_asociada" /></div></div><div class="filters-actions"><button class="btn-cyan" type="button" @click="handleSearch" :disabled="isLoading">{{ isLoading ? 'BUSCANDO...' : 'BUSCAR' }}</button><button class="btn-cyan-outline" type="button" @click="resetForm">LIMPIAR</button></div></div>
      </section>
