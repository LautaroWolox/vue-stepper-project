  ...
  <div v-if="show" class="floating-popup ot-note-popup" :class="{ expanded: isExpanded }" :style="!isExpanded ? { top: pos.y + 'px', left: pos.x + 'px' } : {}">
    ...
    <div class="popup-body ot-body">
      ...
      <section class="tab-box">
        <div v-show="activeTab === 'actividades'"><h4>Actividades Originales</h4><PopupGrid :columns="actividadesCols" :rows="actividades" empty="No hay actividades originales." /></div>
        <div v-show="activeTab === 'base'"><h4>Base Instalada</h4><PopupGrid :columns="baseCols" :rows="baseInstalada" empty="No hay base instalada para mostrar." /></div>
        <div v-show="activeTab === 'historial'">
          <div class="section-title"><h4>Historial del domicilio</h4><button class="btn-cyan" @click="exportHistorial">EXPORTAR HISTORIAL COMPLETO</button></div>
          <div class="table-wrap historial-wrap"><table class="fm-grid popup-grid"><thead><tr><th class="arrow-col"></th><th v-for="column in historialCols" :key="column.field"><button class="grid-sort-header" type="button" @click="toggleHistSort(column.field)"><span>{{ column.label }}</span><span class="sort-arrows" :class="{ active: histSort.field === column.field }"><span :class="{ on: histSort.field === column.field && histSort.direction === 'asc' }">▲</span><span :class="{ on: histSort.field === column.field && histSort.direction === 'desc' }">▼</span></span></button></th></tr></thead><tbody><tr v-for="hist in sortedHistorial" :key="hist.ot"><td class="arrow-col"><button class="arrow-btn" @click="openHistorial(hist)"><span class="material-icons">arrow_right</span></button></td><td class="ot-link">{{ hist.ot }}</td><td>{{ hist.creacion }}</td><td>{{ hist.cierre }}</td><td>{{ hist.acta }}</td><td>{{ hist.estado }}</td><td>{{ hist.contra }}</td><td>{{ hist.red }}</td></tr><tr v-if="sortedHistorial.length === 0"><td class="empty-row" :colspan="historialCols.length + 1">No hay resultados</td></tr></tbody></table></div>
        </div>
        <div v-show="activeTab === 'materiales'"><div class="section-title"><h4>Materiales</h4><button class="btn-cyan" @click="consultarMateriales">CONSULTAR MATERIALES</button></div><PopupGrid :columns="materialCols" :rows="materiales" empty="No se encontraron materiales." /></div>
      ...
      <section class="resultantes"><h4>Actividades Resultantes</h4><PopupGrid :columns="resultCols" :rows="resultantes" selectable empty="No hay actividades resultantes." /></section>
    </div>
    ...
    <HistorialActivitiesPopup :show="showSubgrilla" :data="selectedHistorial" @close="showSubgrilla = false" />
...
import HistorialActivitiesPopup from '../../popup/HistorialActivitiesPopup.vue'
...
const PopupGrid = defineComponent({
  props: { columns: { type: Array, default: () => [] }, rows: { type: Array, default: () => [] }, empty: { type: String, default: 'No hay resultados' }, selectable: { type: Boolean, default: false } },
  setup(props) {
    const sortField = ref('')
    ...
    const sortedRows = computed(() => { const data = [...props.rows]; if (!sortField.value || !sortDirection.value) return data; return data.sort((a, b) => { const left = String(a[sortField.value] ?? ''); const right = String(b[sortField.value] ?? ''); const result = left.localeCompare(right, 'es', { numeric: true, sensitivity: 'base' }); return sortDirection.value === 'asc' ? result : -result }) })
    ...
    const renderSortHeader = (column) => h('button', { class: 'grid-sort-header', type: 'button', onClick: () => toggleSort(column.field) }, [h('span', column.label), h('span', { class: ['sort-arrows', { active: sortField.value === column.field }] }, [h('span', { class: { on: sortField.value === column.field && sortDirection.value === 'asc' } }, '▲'), h('span', { class: { on: sortField.value === column.field && sortDirection.value === 'desc' } }, '▼')])])
    return () => h('div', { class: 'table-wrap' }, [h('table', { class: 'fm-grid popup-grid' }, [h('thead', [h('tr', [props.selectable ? h('th', { class: 'check-col' }, [h('input', { type: 'checkbox', checked: allSelected.value, onChange: toggleAll })]) : null, ...props.columns.map((column) => h('th', { key: column.field }, [renderSortHeader(column)]))])]), h('tbody', sortedRows.value.length ? sortedRows.value.map((row, index) => h('tr', { key: index, class: { selected: row.selected } }, [props.selectable ? h('td', { class: 'check-col' }, [h('input', { type: 'checkbox', checked: !!row.selected, onChange: (event) => toggleRow(row, event) })]) : null, ...props.columns.map((column) => h('td', { key: column.field }, row[column.field] ?? ''))])) : [h('tr', [h('td', { class: 'empty-row', colspan: props.columns.length + (props.selectable ? 1 : 0) }, props.empty)])])])])
  }
})
...
const pos = ref({ x: 60, y: 60 }), isDragging = ref(false), histSort = ref({ field: '', direction: '' })
...
const actividades = ref(otOriginalActivitiesMock.map((item) => ({ ...item }))), baseInstalada = ref([]), materiales = ref([]), historial = ref(otHistorialDomicilioMock.map((item) => ({ ...item, actividades: item.actividades.map((activity) => ({ ...activity })) }))), resultantes = ref(otResultantesMock.map((item) => ({ ...item }))), siniestro = ref({ nroEhs: '', nroOi: '' })
const sortedHistorial = computed(() => { const data = [...historial.value]; if (!histSort.value.field || !histSort.value.direction) return data; return data.sort((a, b) => { const left = String(a[histSort.value.field] ?? ''); const right = String(b[histSort.value.field] ?? ''); const result = left.localeCompare(right, 'es', { numeric: true, sensitivity: 'base' }); return histSort.value.direction === 'asc' ? result : -result }) })
const toggleHistSort = (field) => { if (histSort.value.field !== field) histSort.value = { field, direction: 'asc' }; else if (histSort.value.direction === 'asc') histSort.value = { field, direction: 'desc' }; else histSort.value = { field: '', direction: '' } }
