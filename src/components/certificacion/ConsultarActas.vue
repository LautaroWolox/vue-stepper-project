  ...
  <div class="consulta-actas-page">
    <div v-if="currentView === 'search'">
      <section class="fm-panel">
        ...
        <div class="fm-panel-content filters-content" v-show="openFilters">
          ...
          <div class="filters-row third-row">
            <div class="fm-field"><label>Acta</label><input type="text" class="form-control" v-model.trim="form.acta"></div>
      ...
      <div v-if="isLoading" class="main-loader-container">
        <svg class="svg-coder" viewBox="0 0 200 150" xmlns="http://www.w3.org/2000/svg"><rect x="10" y="120" width="180" height="8" fill="#B0BEC5" rx="4"/><rect x="55" y="95" width="10" height="25" fill="#78909C"/><rect x="40" y="115" width="40" height="5" fill="#607D8B" rx="2"/><rect x="20" y="35" width="80" height="60" fill="#263238" rx="5"/><rect x="25" y="40" width="70" height="45" fill="#00BCD4" rx="2"/></svg>
        <span>Recuperando Actas del sistema...</span>
      </div>
...
const listas = {
  provincias: provinciasCertificacion,
  contratistas: ['TELEWORLD ARGENTINA S.A.', 'AGB SRL', 'INTERCATV S.R.L.', 'TECHFAR-TEL S.R.L.', 'ALL VISION S.A.', 'NET AND WORK S.A.', 'GREENIN S.A.S.', 'BULLS', 'DUNKEL'],
  sociedades: ['Telecom Argentina S.A.', 'La Capital Cable S.A.', 'Núcleo S.A.', 'Bersabel S.A.'],
  tipos_contrato: ['Eventos', 'Eventos SMB', 'Instalación Poste FTTH', 'WIRELESS', 'DTH', 'Siniestros']
}
...
<style scoped>
.consulta-actas-page { color: #111; font-size: 14px; }
.fm-panel { border: 1px solid #d8d8d8; background: #fff; margin-bottom: 8px; }
.fm-panel-header { height: 32px; display: flex; align-items: center; justify-content: space-between; padding: 0 10px; background: #f7f7f7; border-bottom: 1px solid #d8d8d8; cursor: pointer; font-size: 14px; font-weight: 400; }
...
.third-row { grid-template-columns: repeat(2, minmax(220px, 1fr)) 2fr; }
.fm-field label { display: block; margin-bottom: 7px; font-weight: 700; font-size: 13px; }
.form-control { width: 100%; height: 30px; border: 1px solid #bdbdbd; border-radius: 3px; padding: 4px 8px; background: #fff; font-size: 13px; }
.form-control:focus { outline: none; border-color: #00bcd4; box-shadow: 0 0 0 2px rgba(0,188,212,.14); }
.filters-actions { display: flex; justify-content: center; gap: 8px; padding: 18px 0 22px; }
.btn-cyan, .btn-cyan-outline { border-radius: 18px; padding: 8px 18px; font-size: 13px; font-weight: 400; cursor: pointer; transition: background-color .18s ease, border-color .18s ease, box-shadow .18s ease, color .18s ease; }
...
.btn-cyan:disabled { opacity: .65; cursor: not-allowed; }
.btn-cyan-outline { background: #fff; border: 1px solid #00a9bd; color: #00a9bd; }
.btn-cyan-outline:hover { background: #e0f7fa; color: #006f7f; box-shadow: 0 4px 10px rgba(0,143,161,.14); }
