<template>
  <div class="floating-popup" style="width: 480px;" :style="{ top: pos.y + 'px', left: pos.x + 'px' }" v-if="show">
    ...
    <div class="popup-header" style="background: white; color: #546e7a; border-bottom: 1px solid #eceff1;" @mousedown="startDrag">
      <div class="popup-title" style="font-weight: normal; font-size: 16px;">Edición Jobtype-Contrato</div>
    ...
    <div class="popup-body" style="background: white; padding: 25px;">
      <div style="display: flex; gap: 20px;">
        <div class="form-group" style="flex: 1;">
          ...
          <input type="text" class="form-control" :value="data?.tarea" disabled style="background: #fff; color: #9e9e9e; border: 1px solid #e0e0e0;">
        </div>

        <div class="form-group" style="flex: 1;">
          <label style="color: #000; font-size: 13px;">Contrato</label>
          <input type="text" class="form-control" :value="data?.contrato" disabled style="background: #fff; color: #9e9e9e; border: 1px solid #e0e0e0; margin-bottom: 10px;">
          <input type="text" class="form-control" v-model="editData.contrato" style="border-radius: 4px; border-color: #00bcd4;">
        </div>
      </div>
    </div>
...
const editData = reactive({ contrato: '' })
...
const save = () => {
  if(editData.contrato.trim() !== '') {
    emit('update', { id: props.data.id, contrato: editData.contrato })
  } else {
    emit('update', { id: props.data.id, contrato: props.data.contrato })
  }
}
