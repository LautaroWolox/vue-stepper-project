<template>
  <div class="floating-popup" style="width: 450px;" :style="{ top: pos.y + 'px', left: pos.x + 'px' }" v-if="show">
    <div class="popup-header" @mousedown="startDrag">
      <div class="popup-title"><span class="material-icons">history</span> Historial de Cambios: {{ data?.ot.nroot }}</div>
    </div>
    <div class="popup-body">
      <p style="color: #455a64; margin-top: 0;">Materiales registrados en la última modificación:</p>
      <table class="popup-grid">
        <thead>
          <tr><th>Material Modificado/Agregado</th><th>Cant. Final</th></tr>
        </thead>
        <tbody>
          <tr v-for="(mat, i) in data?.materials" :key="i">
            <td>{{ mat.material }}</td>
            <td style="font-weight: bold; color: #00838f;">{{ mat.cantidad }}</td>
          </tr>
        </tbody>
      </table>
    </div>
